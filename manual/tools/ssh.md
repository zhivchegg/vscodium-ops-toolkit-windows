...
## Быстрое подключение к частым хостам

Для постоянной работы с одними и теми же хостами удобно завести персональные алиасы. В MSYS2 это делается внутри домашнего профиля `VSCodium-portable/msys64/home/<имя_пользователя>/`.

### Алиасы в `~/.bash_aliases`

Создай файл `~/.bash_aliases`:

```bash
# SSH
alias s-node1='ssh user@node1.example.com'
alias s-jump='ssh user@bastion.example.com'

# kubectl exec в частые pod
alias k-app-logs='kubectl logs -n my-ns deployment/app --tail=100 -f'
alias k-app-shell='kubectl exec -it -n my-ns deployment/app -- bash'

# Скачивание логов
alias get-logs='rsync -avz user@host:/var/log/app/ ./logs/'
```
...
Чтобы алиасы загружались автоматически, убедись, что в `~/.bashrc` есть блок:
...
Для хостов, к которым подключаешься по SSH, создай `~/.ssh/config`:
...
> **Важно:** `~/.bash_aliases` и `~/.ssh/config` хранятся внутри MSYS2-профиля (`msys64/home/<user>/`). Они не входят в состав сборки и не попадают в архив — это персональные файлы. При переносе сборки на другую машину их нужно будет перенести отдельно или создать заново.
