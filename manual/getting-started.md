...
## Настройка Python, Java и Git

Python и Java не входят в сборку. **Git входит** как часть MSYS2, но вы можете переключиться на уже установленный Git для Windows.

1. Скопируйте готовые portable-папки Python и/или JDK на целевую машину.
2. Закройте VSCodium.
3. Запустите **`configure-runtime.cmd`**.
4. Выберите папки Python, Java и/или Git, нажмите **Test**, затем **Save**.
5. Снова запустите VSCodium через `start-vscodium.cmd`.

Если вы укажете папку Git (например, `C:\Program Files\Git`), то и VSCodium, и терминал будут использовать один и тот же Git. Это удобнее, если у вас уже настроены `.gitconfig` и SSH-ключи в Windows-профиле.
...
- `Configure Runtime.lnk` — настройка Python/Java/Git.

После перемещения папки запустите `create-shortcuts.cmd` заново, чтобы пути в ярлыках обновились.

## Git внутри сборки

**В сборку включён Git из MSYS2**, он работает в терминале VSCodium. Это **не** тот Git, который может быть установлен в Windows отдельно.
...
- Если на машине уже установлен Git для Windows, настройте его через `configure-runtime.cmd` — так Source Control и терминал будут использовать одну версию Git и один профиль (`C:\Users\<имя>\.gitconfig`, `C:\Users\<имя>\.ssh`).
